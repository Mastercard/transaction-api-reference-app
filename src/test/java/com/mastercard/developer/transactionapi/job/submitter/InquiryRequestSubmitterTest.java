...
class InquiryRequestSubmitterTest {
    ...
    private ApiResponse<InquiryResponseInquiryResponseV01> testResponse = TestRequestResponseGenerator.getInquiryResponse(HttpStatus.SC_OK);

    private InquiryResponseStatus testResponseStatus = TestRequestResponseGenerator.getInqResponseItem(TEST_CORRELATION_ID,TEST_CUSTOMER_CONTEXT_KEY,HttpStatus.SC_OK);
    private InquiryResponseStatus testResponseStatusError = TestRequestResponseGenerator.getInqResponseItemWithErrors(TEST_CORRELATION_ID,TEST_CUSTOMER_CONTEXT_KEY,HttpStatus.SC_BAD_REQUEST);
    ...
    void whenSubmitRequest_thenVerifyRequestSent() {
        // call
        inquiryRequestSubmitter.submitRequest(TEST_CUSTOMER_CONTEXT_KEY, testRequest);

        // verify
        verify(transactionApiClient).submitInquiryRequest(TEST_CUSTOMER_CONTEXT_KEY, testRequest);
    }

    @Test
    void givenApiResponse_whenToResponseStatus_thenReturnResponseStatus() {
        // call
        ResponseStatus<InquiryResponseInquiryResponseV01> actual = inquiryRequestSubmitter.toResponseStatus(TEST_CUSTOMER_CONTEXT_KEY, testResponse);
    ...
    void givenApiException_whenToResponseStatus_thenReturnResponseStatus() {
        // setup
        InquiryResponseInquiryResponseV01 errorPayload = testResponseStatusError.getErrors().get(0).getPayload().getInquiryResponse();

        String errorJson = getErrorWrapper(getInqErrorPayload()).toJson();
        ApiException exception = new ApiException("Bad Request",HttpStatus.SC_BAD_REQUEST,responseMap,errorJson);

        // call
        ResponseStatus<InquiryResponseInquiryResponseV01> actual = inquiryRequestSubmitter.toResponseStatus(TEST_CUSTOMER_CONTEXT_KEY, exception);

        // verify
        assertThat(actual.getHttpStatus()).isEqualTo(testResponseStatusError.getHttpStatus());
        assertThat(actual.getErrors().get(0).getPayload().getInquiryResponse()).isEqualTo(errorPayload);
    ...
    void givenSubmitter_whenGenerateRequest_thenReturnRequestPayload() {
        ...
        // call
        InquiryInitiationInquiryInitiationV01 actual = inquiryRequestSubmitter.generateRequest();
    ...
    void givenSubmitter_whenRequestToJson_thenReturnJSONPayload() {
        ...
        // call
        String actual = inquiryRequestSubmitter.requestToJson(testRequest);
