...
class AuthorisationAdviceRequestSubmitterTest {

    @Spy
    @InjectMocks
    private AuthorisationAdviceRequestSubmitter authorisationAdviceRequestSubmitter;
    ...
    private AuthorisationInitiationAuthorisationInitiationV02 testRequest = TestRequestResponseGenerator.getTestAuthorisationAdviceInitiationV02();

    private ApiResponse<AuthorisationResponseAuthorisationResponseV02> testResponse = TestRequestResponseGenerator.getAuthorisationAdviceResponse(HttpStatus.SC_OK);

    private AuthorisationAdviceResponseStatus testResponseStatus = TestRequestResponseGenerator.getAuthAdviceResponseItem(TEST_CORRELATION_ID,TEST_CUSTOMER_CONTEXT_KEY,HttpStatus.SC_OK);
    private AuthorisationAdviceResponseStatus testResponseStatusError = TestRequestResponseGenerator.getAuthAdviceResponseItemWithErrors(TEST_CORRELATION_ID,TEST_CUSTOMER_CONTEXT_KEY,HttpStatus.SC_BAD_REQUEST);
    ...
    void whenSubmitRequest_thenVerifyRequestSent() {
        // call
        authorisationAdviceRequestSubmitter.submitRequest(TEST_CUSTOMER_CONTEXT_KEY, testRequest);

        // verify
        verify(transactionApiClient).submitAuthorisationAdviceRequest(TEST_CUSTOMER_CONTEXT_KEY, testRequest);
    }

    @Test
    void givenApiResponse_whenToResponseStatus_thenReturnResponseStatus() {
        // call
        ResponseStatus<AuthorisationResponseAuthorisationResponseV02> actual = authorisationAdviceRequestSubmitter.toResponseStatus(TEST_CUSTOMER_CONTEXT_KEY, testResponse);
    ...
    void givenApiException_whenToResponseStatus_thenReturnResponseStatus() {
        ...
        // call
        ResponseStatus<AuthorisationResponseAuthorisationResponseV02> actual = authorisationAdviceRequestSubmitter.toResponseStatus(TEST_CUSTOMER_CONTEXT_KEY, exception);
    ...
    void givenSubmitter_whenGenerateRequest_thenReturnRequestPayload() {
        ...
        // call
        AuthorisationInitiationAuthorisationInitiationV02 actual = authorisationAdviceRequestSubmitter.generateRequest();
    ...
    void givenSubmitter_whenRequestToJson_thenReturnJSONPayload() {
        // setup
        String expected = "{\"hdr\":{\"msgFctn\":\"ADVC\",\"prtcolVrsn\":\"00.00.00\"";

        // call
        String actual = authorisationAdviceRequestSubmitter.requestToJson(testRequest);
