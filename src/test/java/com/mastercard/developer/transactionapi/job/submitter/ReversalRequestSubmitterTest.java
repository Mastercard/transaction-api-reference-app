...
class ReversalRequestSubmitterTest {
    ...
    private ApiResponse<ReversalResponseReversalResponseV02> testResponse = TestRequestResponseGenerator.getReversalResponse(HttpStatus.SC_OK);

    private ReversalResponseStatus testResponseStatus = TestRequestResponseGenerator.getRevResponseItem(TEST_CORRELATION_ID,TEST_CUSTOMER_CONTEXT_KEY,HttpStatus.SC_OK);
    private ReversalResponseStatus testResponseStatusError = TestRequestResponseGenerator.getRevResponseItemWithErrors(TEST_CORRELATION_ID,TEST_CUSTOMER_CONTEXT_KEY,HttpStatus.SC_BAD_REQUEST);
    ...
    void whenSubmitRequest_thenVerifyRequestSent() {
        // call
        reversalRequestSubmitter.submitRequest(TEST_CUSTOMER_CONTEXT_KEY, testRequest);

        // verify
        verify(transactionApiClient).submitReversalRequest(TEST_CUSTOMER_CONTEXT_KEY, testRequest);
    }

    @Test
    void givenApiResponse_whenToResponseStatus_thenReturnResponseStatus() {
        // call
        ResponseStatus<ReversalResponseReversalResponseV02> actual = reversalRequestSubmitter.toResponseStatus(TEST_CUSTOMER_CONTEXT_KEY, testResponse);
    ...
    void givenApiException_whenToResponseStatus_thenReturnResponseStatus() {
        // setup
        ReversalResponseReversalResponseV02 errorPayload = testResponseStatusError.getErrors().get(0).getPayload().getReversalResponse();

        String errorJson = getErrorWrapper(getRevErrorPayload()).toJson();
        ApiException exception = new ApiException("Bad Request",HttpStatus.SC_BAD_REQUEST,responseMap,errorJson);

        // call
        ResponseStatus<ReversalResponseReversalResponseV02> actual = reversalRequestSubmitter.toResponseStatus(TEST_CUSTOMER_CONTEXT_KEY, exception);

        // verify
        assertThat(actual.getHttpStatus()).isEqualTo(testResponseStatusError.getHttpStatus());
        assertThat(actual.getErrors().get(0).getPayload().getReversalResponse()).isEqualTo(errorPayload);
    ...
    void givenSubmitter_whenGenerateRequest_thenReturnRequestPayload() {
        ...
        // call
        ReversalInitiationReversalInitiationV02 actual = reversalRequestSubmitter.generateRequest();
    ...
    void givenSubmitter_whenRequestToJson_thenReturnJSONPayload() {
        // setup
        String expected = "{\"hdr\":{\"msgFctn\":\"REQU\",\"prtcolVrsn\":\"auths\"";

        // call
        String actual = reversalRequestSubmitter.requestToJson(testRequest);
